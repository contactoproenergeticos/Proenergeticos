...
const Header: React.FC<HeaderProps> = ({ seccion, setSeccion }) => {
    ...
    <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20 md:h-28 lg:h-32">
          ...
          {/* SECCIÓN LOGOTIPO: MEJORADA PARA TABLET */}
          <div 
          ...
          <div className="hidden lg:flex flex-col items-end gap-3 lg:gap-4">
            <button
              ...
              className="bg-[#E30613] text-white font-black px-6 lg:px-8 py-2 rounded-full hover:bg-gray-900 transition-all duration-300 text-[10px] lg:text-xs tracking-[0.2em] uppercase shadow-lg active:scale-95"
            >
          ...
          {/* CONTROLES PARA MÓVIL Y TABLET (HASTA 1023px) */}
          ...
          <>
            ...
            >
                    ...
                    <li key={option.id}>
                      <button
                        ...
                        className={`w-full py-4 text-2xl font-black uppercase italic tracking-tighter transition-all duration-300 flex items-center justify-end gap-4 ${
                          seccion === option.id ? 'text-[#E30613]' : 'text-gray-600 hover:text-[#E30613]'
                        }`}
                      >
                        {option.label}
                        {seccion === option.id && <div className="w-2 h-2 bg-[#E30613] rounded-full" />}
                      </button>
                    </li>
                  ))}
                </ul>
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50">
                <button
                  onClick={() => handleNavClick('facturacion')}
                  className="w-full bg-[#E30613] text-white font-black py-4 rounded-xl uppercase tracking-widest text-sm shadow-lg active:scale-95"
                >
                  Facturación en Línea
                </button>
              </div>
